...
class MatchingExperiment(BaseExperiment):
    ...
    @torch.no_grad()
    def forward(self, preprocessed_inputs1, preprocessed_inputs2) -> Tuple[Tensor, Tensor]:
        ...
        if patch_tokens1.shape[1] > patch_tokens2.shape[1]:
            raise ValueError(f"Expected image1 to have <= patches than image2, got N1={patch_tokens1.shape[1]} > N2={patch_tokens2.shape[1]}.")
        ...
        ar = torch.arange(N1, device=mapping.device).unsqueeze(0).expand(B, -1)
